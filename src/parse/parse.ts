...
export type ITOKENKEYS = keyof typeof TOKENTYPESGlobal;
...
export const parse: IParse = {
    recur() {
        ...
        function add(name: string, min?: number, max?: number) {
            ...
            values = applyMin && min ? [min] : applyMax && max ? [max] : values;
            const length = values.length;
            for (i = 0; i < length; i += 1) {
                const val = values[i];
                if ((<any>cur)[curName].indexOf(val) < 0) {
                    (<any>cur)[curName].push(val);
                }
            }
        ...
        return {
            ...
            customModifier: function (id: string, ...vals: number[]) {
                const custom = Object.keys(later.modifier).includes(id);
    ...
    cron(expr: string, hasSeconds: boolean) {
        ...
        function parse(
            ...
            // parse xW
            else if ((value = getValue(<any>item.replace('W', ''), offset, max)) !== null) {
                addWeekday(schedules, s, curSched, value);
            }
            // parse xL
            else if ((value = getValue(<any>item.replace('L', ''), offset, max)) !== null) {
            ...
            for (field in FIELDS) {
                ...
                if (component && component !== '*' && component !== '?') {
                    ...
                    var i,
                        length = items.length;
                    for (i = 0; i < length; i++) {
                        parse(items[i], schedule, <any>field, f[0], f[1], f[2]);
                    }
                }
            }
