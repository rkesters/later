...
import { compileFactory, ICompiledSchedule, IScheduleDef } from './compile';
...
export function scheduleFactory(later: Base) {
    const compile = compileFactory(later);
    return function (scheduled: ISchedule): IScheduleResult {
        ...
        for (let i = 0; i < schedulesLen; i++) {
            schedules.push(compile(scheduled.schedules[i]));
        }

        for (let j = 0; j < exceptionsLen; j++) {
            exceptions.push(compile((<IScheduleDef[]>scheduled.exceptions)[j]));
        }
        ...
        function getInstances(
            ...
            let compare = compareFn(dir), // encapsulates difference between directions
                ...
                results: Array<LaterDate | Array<LaterDate | undefined> | undefined> = [],
                isForward = dir === 'next',
                lastResult: (LaterDate | undefined)[] | undefined,
            ...
            while (maxAttempts-- && loopCount && (next = findNext(schedStarts, compare)) && next !== NEVER) {
                ...
                if (isRange) {
                    ...
                    const r: (LaterDate | undefined)[] = isForward
                        ? [
                              new LaterDate(Math.max(startDate.getTime(), next.getTime())),
                              end
                                  ? new LaterDate(
                                        endDate ? Math.min(end.getTime(), endDate.getTime()) : end,
                                    )
                                  : undefined,
                          ]
                        : [
                                  ...
                                  ? new LaterDate(
                                        endDate
                                            ? Math.max(endDate.getTime(), end.getTime() + SEC)
                                            : end.getTime() + SEC,
                                    )
                                  : undefined,
                              new LaterDate(Math.min(startDate.getTime(), next.getTime() + SEC)),
                          ];
        ...
        function setRangeStarts(
            dir: DIRECTION,
            schedArr: ICompiledSchedule[],
            rangesArr: Array<number | (LaterDate | undefined)[] | LaterDate | undefined>,
            startDate: LaterDate,
        ) {
            var compare = compareFn(dir);
        ...
        function calcEnd(
            ...
            for (; i < len; i++) {
                ...
                if (start && start.getTime() === startDate.getTime()) {
                    let end: LaterDate | undefined = schedArr[i].end(dir, start);
                    end = end === NEVER ? undefined : end;
         ...
         * @param {String} dir: The direction to use, either 'next' or 'prev'
         */
        function compareFn(dir: DIRECTION): (a: Date | undefined, b: Date | undefined) => boolean {
            return dir === 'next'
                ? function (a, b) {
                      return !!a && (!b || a.getTime() > b.getTime());
                  }
                : function (a, b) {
                      return !!b && (!a || b.getTime() > a.getTime());
                  };
        }
         ...
         */
        function findNext(arr: LaterDate[], compare: (a: LaterDate, b: LaterDate) => boolean): LaterDate {
            let next: LaterDate = arr[0];

            let i = 1,
                len = arr.length;
            for (; i < len; i++) {
                if (arr[i] && arr[i] != NEVER && compare(next, arr[i])) {
                    next = arr[i];
                }
            }
