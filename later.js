later = (function () {
    ...
    later.day = later.D = {
        ...
        start: function (d) {
            return (
                d.DStart ||
                (d.DStart = later.date.next(later.Y.val(d), later.M.val(d), later.D.val(d)))
            );
        },
        end: function (d) {
            return (
                d.DEnd || (d.DEnd = later.date.prev(later.Y.val(d), later.M.val(d), later.D.val(d)))
            );
        },
        next: function (d, val) {
    ...
    later.dayOfWeekCount = later.dc = {
        ...
        isValid: function (d, val) {
            return (
                later.dc.val(d) === val || (val === 0 && later.D.val(d) > later.D.extent(d)[1] - 7)
            );
        },
        extent: function (d) {
