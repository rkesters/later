later = function () {
    ...
    later.array.nextInvalid = function (val, values, extent) {
        var min = extent[0], max = extent[1], len = values.length,
            zeroVal = values[len - 1] === 0 && min !== 0 ? max : 0, next = val, i = values.indexOf(val), start = next;
    ...
    later.day = later.D = {
        name: "day", range: 86400, val: function (d) {
    ...
    later.dayOfWeekCount = later.dc = {
        name: "day of week count", range: 604800, val: function (d) {
    ...
    later.dayOfWeek = later.dw = later.d = {
        name: "day of week", range: 86400, val: function (d) {
    ...
    later.dayOfYear = later.dy = {
        name: "day of year", range: 86400, val: function (d) {
    ...
    later.hour = later.h = {
        name: "hour", range: 3600, val: function (d) {
    ...
    later.minute = later.m = {
        name: "minute", range: 60, val: function (d) {
            ...
            var m = later.m.val(d), s = later.s.val(d), inc = val > 59 ? 60 - m : val <= m ? 60 - m + val : val - m,
    ...
    later.month = later.M = {
        name: "month", range: 2629740, val: function (d) {
    ...
    later.second = later.s = {
        name: "second", range: 1, val: function (d) {
            ...
            var s = later.s.val(d), inc = val > 59 ? 60 - s : val <= s ? 60 - s + val : val - s,
    ...
    later.time = later.t = {
        name: "time", range: 1, val: function (d) {
    ...
    later.weekOfMonth = later.wm = {
        name: "week of month", range: 604800, val: function (d) {
    ...
    later.weekOfYear = later.wy = {
        name: "week of year (ISO)", range: 604800, val: function (d) {
            if (d.wy) return d.wy;
            ...
            var year = later.dw.next(later.wy.start(d), 5), dwFirst = later.dw.val(later.Y.start(year)),
    ...
    later.year = later.Y = {
        name: "year", range: 31556900, val: function (d) {
    ...
    later.fullDate = later.fd = {
        name: "full date", range: 1, val: function (d) {
    ...
    later.compile = function (schedDef) {
        ...
        for (var key in schedDef) {
            var nameParts = key.split("_"), name = nameParts[0], mod = nameParts[1], vals = schedDef[key],
            ...
            start: function (dir, startDate) {
                ...
                while (maxAttempts-- && !done && next) {
                    ...
                    for (var i = 0; i < constraintsLen; i++) {
                        var constraint = constraints[i].constraint, curVal = constraint.val(next),
                            extent = constraint.extent(next), newVal = nextVal(curVal, constraints[i].vals, extent);
                ...
                for (var i = constraintsLen - 1; i >= 0; i--) {
                    var constraint = constraints[i].constraint, curVal = constraint.val(startDate),
                        extent = constraint.extent(startDate), newVal = nextVal(curVal, constraints[i].vals, extent),
                        next;
                    if (newVal !== undefined) {
    ...
    later.schedule = function (sched) {
        ...
        var schedules = [], schedulesLen = sched.schedules.length, exceptions = [],
        ...
        function getInstances(dir, count, startDate, endDate, isRange) {
            var compare = compareFn(dir), loopCount = count, maxAttempts = 1e3, schedStarts = [], exceptStarts = [],
                next, end, results = [], isForward = dir === "next", lastResult, rStart = isForward ? 0 : 1,
                rEnd = isForward ? 1 : 0;
    ...
    later.parse.cron = function (expr, hasSeconds) {
        ...
        function add(schedules, sched, name, min, max, inc) {
            var i = min;
            if (!sched[name]) {
                sched[name] = []
            }
        ...
        function addHash(schedules, curSched, value, hash) {
            ...
            add(schedules, schedules[schedules.length - 1], "dc", hash, hash)
        }
        ...
        function parseExpr(expr) {
            var schedule = {schedules: [{}], exceptions: []}, components = expr.replace(/(\s)+/g, " ").split(" "),
    ...
    later.parse.recur = function () {
        var schedules = [], exceptions = [], cur, curArr = schedules, curName, values, every, modifier, applyMin,
            applyMax, i, last;
        ...
        return {
            schedules: schedules, exceptions: exceptions, on: function () {
    ...
    later.parse.text = function (str) {
        ...
        function peek(expected) {
            var scanTokens = expected instanceof Array ? expected : [expected], whiteSpace = /\s+/, token, curInput, m,
                scanToken, start, len;
        ...
        function parseThroughExpr(tokenType) {
            var start = +parseTokenValue(tokenType),
                end = checkAndParse(TOKENTYPES.through) ? +parseTokenValue(tokenType) : start, nums = [];
